...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        input[type="text"], input[type="number"], input[type="email"], textarea { width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px; font-family: inherit; transition: border-color 0.3s; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <form id="configForm">
                <div class="section">
                    <h2 class="section-title">WiFi Station</h2>
                    <div class="form-group"><label><div class="checkbox-group"><input type="checkbox" id="wifiEnabled"><span>Enable WiFi Station</span></div></label></div>
                    <div class="form-group"><label>SSID</label><input type="text" id="wifiSsid" placeholder="Your WiFi network"></div>
                    <div class="form-group"><label>Password</label><input type="password" id="wifiPassword" placeholder="WiFi password"></div>
                    <div class="form-group"><label>Hostname</label><input type="text" id="wifiHostname" placeholder="esp8266-email-alert"></div>
                    <div class="form-group"><label>Access Point SSID</label><input type="text" id="apSsid" placeholder="ESP8266-Email-Alert"></div>
                    <div class="form-group"><label>Access Point Password</label><input type="password" id="apPassword" placeholder="Minimum 8 characters"></div>
                </div>
                <div class="section">
                    ...
                    <div class="form-group"><label>Password</label><input type="password" id="smtpPassword" placeholder="your-app-password"></div>
    ...
    <script>
        ...
        async function loadConfiguration() {
            ...
            document.getElementById("wifiEnabled").checked = data.wifi.enabled;
            document.getElementById("wifiSsid").value = data.wifi.ssid;
            document.getElementById("wifiPassword").value = data.wifi.password;
            document.getElementById("wifiHostname").value = data.wifi.hostname;
            document.getElementById("apSsid").value = data.wifi.ap_ssid;
            document.getElementById("apPassword").value = data.wifi.ap_password;
            ...
            for (let i = 0; i < 4; i++) {
                ...
                gpioContainer.innerHTML += `
                    <div class="gpio-card">
                        <div class="form-group">
                            <label>GPIO ${i + 1} Label</label>
                            <input type="text" id="gpio${i}Label" placeholder="Label">
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label>Pin Number</label>
                                <input type="number" id="gpio${i}Pin" min="0" max="255" placeholder="5">
                            </div>
                            <div class="form-group">
                                <label>Flags</label>
                                <div class="checkbox-group">
                                    <input type="checkbox" id="gpio${i}Enabled">
                                    <span>Enabled</span>
                                </div>
                                <div class="checkbox-group">
                                    <input type="checkbox" id="gpio${i}ActiveHigh">
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="gpio${i}Message" placeholder="Message"></textarea>
                        </div>
                    </div>`;
                ...
                document.getElementById("gpio" + i + "Pin").value = gpio.pin;
        ...
        async function saveConfiguration(e) {
            ...
            const config = { wifi: {}, smtp: {}, gpio: [], alive: {} };
            config.wifi.enabled = document.getElementById("wifiEnabled").checked;
            config.wifi.ssid = document.getElementById("wifiSsid").value;
            config.wifi.password = document.getElementById("wifiPassword").value;
            config.wifi.hostname = document.getElementById("wifiHostname").value;
            config.wifi.ap_ssid = document.getElementById("apSsid").value;
            config.wifi.ap_password = document.getElementById("apPassword").value;

            config.smtp.host = document.getElementById("smtpHost").value;
            config.smtp.port = parseInt(document.getElementById("smtpPort").value, 10);
            ...
            for (let i = 0; i < 4; i++) {
                config.gpio.push({
                    enabled: document.getElementById("gpio" + i + "Enabled").checked,
                    active_high: document.getElementById("gpio" + i + "ActiveHigh").checked,
                    pin: parseInt(document.getElementById("gpio" + i + "Pin").value, 10),
                    label: document.getElementById("gpio" + i + "Label").value,
                    message: document.getElementById("gpio" + i + "Message").value
                });
            }

            config.alive.enabled = document.getElementById("aliveEnabled").checked;
            config.alive.interval_ms = parseInt(document.getElementById("aliveInterval").value, 10);
